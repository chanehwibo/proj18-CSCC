...
如果 `git --version` 提示找不到命令，不影响已经准备好样本目录的功能演示；本手册默认使用已准备好的本地样本。
...
如果 `data\samples\xv6-public` 或 `data\samples\oskernel2024-aabcb` 不存在，不建议在 5 分钟演示中现场处理。请先退出录屏，确认本地样本目录已经提前准备好，再从下一节的 `manifest-audit` 开始演示。

注意：

- 本手册默认样本源码已经存在于 `data\samples`。
...
如果样本目录不完整，`repos` 仍可能是 21，但 warnings 可能提示本地目录缺失。正式录屏前建议确保为 `0 error、0 warning`。
...
- 已经准备在 `data\samples\` 的公开样本，适合稳定录屏演示。
- 任意本地 OS 仓库路径，例如 `D:\repos\student-os`。

历史比对库来自 `data\samples\manifest.json` 和本地 `data\samples\` 目录。输入仓库不需要提交到本项目仓库；提交的是样本清单和最终生成的报告。若输入仓库本身也位于 `data\samples\`，系统会跳过它自身，只和其他历史样本比对。

### 3.2 用比赛作品样本作为输入仓库

下面这组命令更像评委关心的查重场景：把已经准备好的 `data\samples\oskernel2024-aabcb` 当作待检测输入仓库，直接和历史样本库做对比。5 分钟演示里直接进入画像、对比、报告和证据展示。
...
本手册演示固定使用 `oskernel2024-aabcb`，不在录屏中切换新的输入仓库。

建议旁边红字说明：

```text
系统输入的是一个待检测 OS 仓库路径，历史样本库只作为对比基线；第三方源码不随项目提交，最终报告和 manifest 可说明样本来源。
```
...
- 不要打开 `.env`。
- 不要展示 API key。
- 不要把 `.env`、`data\llm_cache`、`data\reports\prompts`、`data\reports\tmp`、`data\profiles`、`data\samples` 提交到仓库；`data\reports\describe`、`data\reports\compare`、`data\reports\html` 中的最终演示报告可以提交。
...
- 不在录屏中现场修复样本目录。
- 先停止演示，恢复提前准备好的 `data\samples` 本地样本目录。
- 再回到第 1.5 节确认样本目录，然后继续从 `manifest-audit` 开始。
...
- manifest 中新增样本但本地目录尚未准备好。

处理方式：

1. 先确认 `data\samples` 下样本目录是否完整。
2. 不在录屏现场修复样本目录，先恢复提前准备好的本地样本目录。
