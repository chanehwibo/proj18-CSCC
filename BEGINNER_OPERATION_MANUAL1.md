...
| `data\reports\describe\xv6-public.md` | 单仓库描述报告 | 否 |
| `data\reports\compare\xv6-public_vs_history.md` | 历史样本对比报告 | 否 |
| `data\reports\html\xv6-public.describe.html` | 描述报告 HTML 证据视图 | 否 |
| `data\reports\html\xv6-public_vs_history.html` | 对比报告 HTML 证据视图 | 否 |
...
```text
五个核心产物全部生成，且都位于 ignored 的 data 目录，不会泄露本地样本或缓存。
```
...
- 不要打开 `.env`。
- 不要展示 API key。
- 不要把 `.env`、`data\llm_cache`、`data\reports`、`data\profiles`、`data\samples` 提交到仓库。
...
| 1:10-1:50 | PowerShell | 运行 `demo --html` | `一条命令完成画像、描述、对比和 HTML 报告` |
