...
.ks-floattip{position:fixed;z-index:9999;max-width:300px;padding:9px 12px;border-radius:8px;background:var(--ink);color:var(--card);font-size:12px;font-weight:500;line-height:1.5;box-shadow:0 10px 28px rgba(0,0,0,.35);opacity:0;visibility:hidden;transition:opacity .12s ease;pointer-events:none;}
.ks-floattip.show{opacity:1;visibility:visible;}
...
.basework-lbl{display:inline-flex;align-items:center;gap:6px;font-size:13px;color:var(--brand-dark);font-weight:600;background:var(--brand-soft);border-radius:8px;padding:2px 10px 2px 12px;}
.basework-lbl select{font-weight:600;}
...
"""
    ...
    def _history_filter_panel(self, site_data: dict[str, Any], years: list[dict[str, Any]]) -> str:
        ...
        return f'''\n  <section class="history-filter"><h2>历史作品对比筛选器</h2>\n    <div class="history-controls"><label class="basework-lbl">待测作品 <select id="histBaseWork" onchange="applyHistoryFilters()"></select></label><input id="histQ" type="text" placeholder="搜索历史项目 / 学校 / 来源 / 语言..." oninput="applyHistoryFilters()"><select id="histYear" onchange="applyHistoryFilters()">{year_opts}</select><select id="histAward" onchange="applyHistoryFilters()">{award_opts}</select><select id="histLang" onchange="applyHistoryFilters()">{lang_opts}</select><select id="histDim" onchange="applyHistoryFilters()">{dim_opts}</select><select id="histSim" onchange="applyHistoryFilters()"><option value="all">全部相似度</option><option value="high">高相似</option><option value="medium">中相似</option><option value="low">低相似</option></select><span class="count" id="histCount"></span></div>\n    <div class="history-table-wrap"><table class="history-table"><thead><tr><th>仓库</th><th>名称</th><th>年份</th><th>奖项/来源</th><th>主语言</th><th>LOC</th><th>相似参考<span class="info-tip" tabindex="0" data-tip="所选待测作品与该历史样本的快速相似度估算（语言+38/代码规模接近+32/获奖+12/架构+10），仅用于快速定位复核对象，不等于精确比对结论。">?</span></th></tr></thead><tbody id="historyRows">{"".join(rows)}</tbody></table></div><div class="hint">「相似参考」= 上方所选<b>待测作品</b>与各历史样本的快速相似度估算（按语言、代码规模、架构和来源），用于在 {site_data.get("baseline", {}).get("count", len(repos))} 个历史样本中快速定位复核对象；精确的逐行证据比对见各作品卡片的比较报告。</div>\n  </section>'''
