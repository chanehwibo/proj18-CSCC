...
- 对比历史仓库：2022 Maturin
- 生成时间：2026-07-02T04:57:32.025029+00:00
...
- 待测作品包含 336 个历史样本 2022 Maturin 中未见的独特函数/类型定义（如 `render_busybox_script`, `read_at`, `read_regular_file`, `enter_user`, `read_sector`, `render_scenario`），可能体现独立设计或扩展实现。这是命名级线索，需结合源码语义确认是否构成创新。（置信度：low）
  证据：
  ...
  - `src/fs/ext4.rs:L837-L837`：fn read_at
    代码片段：`fn read_at<D: BlockDevice>(device: &mut D, offset: u64, out: &mut [u8]) -> Result<(), Ext4Error> {`
