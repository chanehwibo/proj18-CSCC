...
- 对比历史仓库：2022 啊队队队
- 生成时间：2026-07-02T05:02:20.380678+00:00
- 参考库边界：只有带官方来源的 `verified_award` 样本才会被视为获奖案例；未核验比赛样本不作为特奖/一等奖背书。

## 历史样本选择

- 2022 啊队队队（来源：赛事历史作品）：画像相似度 score=9.41；同属 ucore-variant 风格; 语言构成相似度 0.97; OS 维度重合度 1.00; 代码规模接近度 0.47
...
- 与 2022 啊队队队 的语言构成不同：待测作品为 {'markdown': 2188, 'build': 354, 'toml': 94, 'rust': 43626, 'asm': 381, 'make': 409}，历史样本为 {'yaml': 19, 'json': 20, 'build': 91, 'markdown': 995, 'text': 19, 'toml': 2, 'rust': 13878, 'asm': 227}。（置信度：medium）
...
- 待测作品代码规模（47,052 LOC）显著大于历史样本 2022 啊队队队（15,251 LOC），可能包含更多功能模块或更完整的实现，但也可能包含非内核代码，需人工确认。（置信度：low）
