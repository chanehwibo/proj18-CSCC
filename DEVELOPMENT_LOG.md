...
## 阶段 29：README 按优秀仓库结构重构

- 日期：2026-06-12
- 目标：参考优秀仓库 `README (1).md` 的比赛项目展示结构，将本项目 README 从工具说明式结构调整为更适合评审浏览的参赛作品说明结构。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| 章节结构 | 改为“基本信息、项目概述、项目目标与规划、项目架构和设计方案、测试与评估、开发文档和演示材料、快速启动、目录索引、致谢”九大章节 |
| 目录导航 | README 目录改为与参考优秀仓库一致的编号式层级，并补充二级章节跳转 |
| 内容重排 | 将原有当前状态、核心能力、参考库覆盖、LLM 配置、证据边界、系统架构、仓库目录、快速开始等内容归并到对应章节 |
| 评审表达 | 增加背景意义、项目定位、行动项、推进情况、测试评估和风险边界说明，使 README 更接近答辩材料入口 |
| 信息保留 | 保留官方 GitLab 链接、学校 LOGO、DeepSeek/LLM 安全约定、忽略本地数据目录和证据优先边界 |
...
## 阶段 36：修正 fetch_repos 完整克隆语义

- 日期：2026-06-13
- 目标：回应“`fetch_repos.py --depth 0` 不是真完整克隆”的审核意见，确保文档中的 `0 表示完整克隆` 与实际 git 参数一致。

### 问题确认

旧实现把 `--depth 0` 转换成 `2**31 - 1`，随后仍执行 `git clone --depth <huge> --single-branch`。这虽然接近拉取很长历史，但仍是 shallow clone，且只拉取单分支，不符合“完整克隆”的严格语义。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| fetch 脚本 | 新增 `normalize_clone_depth()`、`build_clone_args()`，集中表达 clone 参数规则 |
| shallow clone | `depth > 0` 时继续使用 `--depth N --single-branch`，保留默认快速采集能力 |
| full clone | `depth <= 0` 时统一规范化为 `0`，执行 `git clone <url> <repo_dir>`，不再传 `--depth` 和 `--single-branch` |
| 元数据 | `.kernelsage_meta.json` 中 `clone_depth=0` 表示完整克隆，不再出现超大伪深度 |
| 回归测试 | 新增 `tests/test_fetch_repos.py`，覆盖浅克隆参数和非正 depth 的完整克隆参数 |
| 文档同步 | 更新 README、报告审计记录和外部痛点文档中的复现边界说明 |

### 边界说明

- 默认 `--depth 1` 仍用于节省空间和时间，适合样本库快速准备。
- `--depth 0` 明确用于审计或复现实验，需要更多网络时间和磁盘空间。
- 新增回归测试后，完整测试数从 57 个增加到 59 个。

## 阶段 37：加固对比报告证据存在性检查

- 日期：2026-06-13
- 目标：回应“对比报告证据存在性检查偏弱”的审核意见，避免 compare self-check 在没有 root 的情况下把 evidence 字段默认视为有效。

### 问题确认

旧实现中 `Reporter.render_compare()` 调用 `EvidenceChecker().compare_summary(result)` 时没有传入 evidence root；而 `_evidence_exists()` 在 `root is None` 时直接返回 `True`。因此对比报告的 self-check 更接近“是否填写了 evidence 字段”，不能严格确认文件路径和行号存在。对于 compare 场景，单一 root 也不够，因为同一份报告同时引用新仓库和多个历史仓库。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| 数据模型 | `Evidence` 增加 `repo_id`，`CompareResult` 增加 `evidence_roots`，并保持默认值以兼容旧构造和旧缓存 |
| CompareAgent | 生成 compare 结果时记录新仓库/历史仓库的 root 映射，并为 overlap、unique、路径重合、符号重合和片段相似 evidence 标注来源仓库 |
| Self-check | compare self-check 优先按 `repo_id -> root` 校验证据；缺少 root 时不再默认视为有效 |
| 路径边界 | evidence 路径解析后必须仍位于对应仓库 root 内，避免绝对路径或 `..` 逃逸 |
| 回归测试 | 新增 compare 缺 root、compare root 映射和 repo-scoped evidence 校验测试，覆盖同名路径存在于错误仓库时应判为无效的场景 |
| 文档同步 | 更新 README、报告审计记录和外部痛点文档中的证据边界说明 |

### 边界说明

- 描述报告仍按单仓库 root 校验 evidence；对比报告按 evidence 的 `repo_id` 选择对应 root。
- 对没有 `repo_id` 但带 `evidence_roots` 的旧 compare evidence，会尝试在任一 root 下校验，用于兼容旧数据；新生成结果都会带 `repo_id`。
- 新增回归测试后，完整测试数从 59 个增加到 62 个。

## 阶段 38：加固 fetch_repos 目标目录安全边界

- 日期：2026-06-15
- 目标：回应“`fetch_repos.py --reclone` 删除目录前缺少路径安全校验”的内部审查发现，避免外部或误填 manifest 中的 `repo_id` 逃出 `--out` 输出目录。

### 问题确认

`clone_one()` 旧逻辑直接用 `out_dir / repo_id` 作为目标目录。如果 `repo_id` 来自不可信 manifest，理论上可能包含 `..`、路径分隔符或 Windows 盘符形式。配合 `--reclone` 时，脚本会在目标目录存在时执行 `shutil.rmtree(repo_dir)`；虽然当前内置 manifest 没有问题，但脚本边界不够明确。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| repo_id 校验 | 新增 `validate_repo_id()`，仅允许字母数字开头以及字母数字、点、下划线和连字符组成的安全目录名 |
| 路径解析 | 新增 `resolve_repo_dir()`，对 `out_dir` 和目标目录执行 `resolve(strict=False)`，并确认目标目录必须位于输出目录内部 |
| 克隆流程 | `clone_one()` 在任何删除或克隆操作前先解析安全目标目录；发现不安全 `repo_id` 时返回 `failed`，不调用 git，也不删除目录 |
| 回归测试 | 新增路径逃逸和 `--reclone` 拒绝危险 `repo_id` 的测试，确认外部目录中的文件不会被删除 |
| 文档同步 | 更新 README 和报告审计记录中的测试数量，外部痛点文档补充采集脚本安全边界说明 |

### 边界说明

- 当前内置 `data/samples/manifest.json` 的 repo_id 已检查，没有路径分隔符、重复项或缺失 URL。
- 本加固主要面向后续自定义 manifest 或外部 manifest 场景，防止采集脚本从“样本准备工具”变成误删风险点。
- 新增 2 条回归测试后，完整测试数从 62 个增加到 64 个。

## 阶段 39：收紧 profile 证据自检边界

- 日期：2026-06-19
- 目标：修复描述报告 self-check 的两个低频误判边界，避免空 `root_path` 借当前工作目录通过校验，也避免空文件被视为存在第 1 行。

### 问题确认

`EvidenceChecker.profile_summary()` 直接对 `profile.meta.root_path` 调用 `Path()`。当 `root_path` 为空字符串时，`Path("")` 会解析为当前工作目录，导致本应缺少仓库 root 的 profile evidence 可能被当前执行目录中的同名文件误判为有效。另一方面，行号校验使用 `max(line_count, 1)`，空文件也会接受 `L1-L1`，这与“证据必须能回到真实源码行”的语义不一致。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| Self-check | `profile_summary()` 对空 `root_path` 使用 `require_root=True`，缺 root 时 evidence 直接判为无效 |
| 行号校验 | 空文件不再被视为拥有第 1 行，证据行号必须落在真实文件行数范围内 |
| 回归测试 | 新增空 root 和空文件两个边界测试，防止后续回退 |

### 边界说明

- 本次修改只收紧证据存在性校验，不改变 analyzer 生成 finding 的规则。
- 对正常由 collector 生成、带真实 `root_path` 的 profile 没有使用方式变化。
- 新增 2 条回归测试后，完整测试数从 64 个增加到 66 个。

## 阶段 40：补全 fetch 本地样本元数据

- 日期：2026-06-19
- 目标：让 `fetch_repos.py` 写入的 `.kernelsage_meta.json` 保留获奖来源和年份等关键字段，避免样本目录脱离 manifest 后丢失样本可信来源。

### 问题确认

采集脚本原本写入 repo_id、名称、URL、类别、架构、语言、团队等字段，但没有写入 `year`、`source_tier`、`award_level` 和 `award_source_url`。正常从项目内置 manifest 读取时，collector 可以从 manifest 补回这些字段；但如果只复制某个样本目录，或者后续使用不带完整 manifest 的样本根目录，本地 `.kernelsage_meta.json` 不能独立说明该样本是否为已核验获奖案例。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| fetch 元数据 | `.kernelsage_meta.json` 新增 `year`、`source_tier`、`award_level`、`award_source_url` |
| 复现边界 | 单个样本目录复制后仍能保留获奖来源和年份信息 |
| 回归测试 | 新增 `write_repo_meta()` 测试，确认获奖来源字段会写入本地元数据 |

### 边界说明

- 本次修改不改变 clone 参数和采集流程，只补齐落盘元数据。
- `award_source_url` 的真实性仍由 manifest 维护者和后续 manifest audit 负责，本地元数据只负责保留来源记录。
- 新增 1 条回归测试后，完整测试数从 66 个增加到 67 个。

## 阶段 41：compare 历史样本并行构建

- 日期：2026-06-19
- 目标：降低大样本库下 `compare` 首次构建或强制重建历史画像的耗时，新增 `--jobs` 并行 worker 参数，同时保持输出顺序稳定。

### 问题确认

`cmd_compare()` 原本按 `history_root.iterdir()` 串行构建或读取每个历史样本的 `KernelProfile`。在 21 个样本下还能接受，但当样本库扩展到 50/100 个，或者使用 `--rebuild-profile-cache` 时，静态扫描、解析和缓存写入会成为主要耗时。直接并发执行还需要避免完成顺序影响历史候选列表，防止报告在不同机器上不稳定。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| CLI 参数 | `compare` 和 `demo` 新增 `--jobs`，默认 1，保持原串行行为 |
| 历史样本收集 | 新增 `_history_repo_dirs()`，跳过隐藏目录和新仓库，并按仓库名稳定排序 |
| 并行构建 | 新增 `build_history_profiles()`，使用 `ThreadPoolExecutor` 并行构建/读取缓存，再按原排序回填结果 |
| 兼容性 | 内部测试直接构造 `argparse.Namespace` 时用 `getattr(args, "jobs", 1)` 兼容旧调用 |
| 文档与测试 | README 补充 `--jobs` 示例；新增 parser 和历史目录排序测试 |

### 边界说明

- `--jobs` 只影响历史 profile 构建/读取阶段，不改变相似度评分、Top-K 选择规则和报告模板。
- 默认 `--jobs 1` 保持原行为；需要重建缓存或样本库较大时可手动调高。
- 新增 2 条回归测试后，完整测试数从 67 个增加到 69 个。

## 阶段 42：优化 profile cache 指纹性能

- 日期：2026-06-19
- 目标：减少缓存命中前的全仓文件扫描成本，在干净 Git 仓库中优先使用 HEAD 和 tracked 文件数生成快速指纹。

### 问题确认

`ProfileCache.fingerprint()` 原本每次都会 `rglob("*")` 遍历仓库文件并统计大小、数量和最新 mtime。即使 profile 缓存最终命中，大仓库也要先付出一次全仓 stat 成本。直接只看 Git HEAD 又会漏掉未提交修改和未跟踪文件，因此需要把快速路径限制在干净 Git 工作区内。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| 缓存 schema | `CACHE_SCHEMA_VERSION` 升到 `1.9`，避免旧全量指纹和新快速指纹混用 |
| 快速指纹 | 新增 `_git_fast_fingerprint()`，干净 Git 仓库使用 `git-clean:<HEAD>` 和 tracked 文件数 |
| 准确性边界 | `git status --porcelain --untracked-files=normal` 非空时回退 `_full_tree_fingerprint()` |
| 非 Git 兼容 | 无 Git、命令失败或无法读取 tracked 文件数时继续使用全量扫描 |
| 回归测试 | 覆盖干净 Git 快速路径不触发全量扫描、脏仓库回退全量扫描 |

### 边界说明

- 快速路径只优化缓存判定前的指纹生成，不改变 profile 内容和分析规则。
- 工作区存在未提交或未跟踪变化时仍用全量扫描，避免把本地修改误判为缓存命中。
- 新增 2 条回归测试后，完整测试数从 69 个增加到 71 个。

## 阶段 43：把 LLM 审计接入生成链路

- 日期：2026-06-19
- 目标：解决“LLM 审计只是独立命令，`--use-llm` 生成报告后不会自动拦截不合格输出”的可靠性问题。

### 问题确认

旧流程已经提供 `audit-llm-report`，可以人工拿 dry-run prompt 和 LLM 报告做离线审计；但 `describe --use-llm` 和 `compare --use-llm` 在真实生成时只处理 API 失败，不会自动检查模型是否引用了 prompt evidence 之外的路径/行号，也不会在模型出现越权抄袭判断时回退。这样在中转站 API 或模型输出波动场景下，最终落盘报告仍可能带有不合格 LLM 文本。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| LLM prompt | `LLMReportGenerator` 新增 `format_profile_prompt()` 和 `format_compare_prompt()`，生成与 dry-run 一致的完整 system/user prompt 文本 |
| CLI 门禁 | `describe` 和 `compare` 的 `--use-llm` 分支在写入报告前调用本地审计；审计 error 会抛出 `RuntimeError` 并回退到规则版报告 |
| Dry-run 边界 | `--llm-dry-run` 仍只生成 prompt 文件并输出规则版报告，不调用 API，也不执行“报告审计” |
| Demo 参数 | `demo --jobs` 继续向内部 compare 调用传递，避免外层参数被吞掉 |
| 回归测试 | 新增 describe/compare 两条 CLI 测试，模拟 LLM 越界引用并确认最终报告回退到规则版 |
| 文档同步 | README 更新 LLM 自动审计、失败回退和测试数量说明，外层痛点文档补充答辩表述 |

### 边界说明

- 自动审计只拦截 severity 为 error 的问题；缺少推荐章节等 warning 保留为审计提示，不阻断 LLM 报告。
- 规则版报告不受影响，默认不调用在线模型，也不增加 token 成本。
- API 异常、缓存异常和审计失败统一走 fallback，目标是保证最终仍有基于本地证据链的报告落盘。
- 新增 2 条回归测试后，完整测试数从 71 个增加到 73 个。

## 阶段 44：新增 manifest-audit 样本库可信度自检

- 日期：2026-06-19
- 目标：把“样本库是否可信、获奖样本是否有来源、采集结果是否可复核”做成可演示的本地 CLI 自检能力。

### 问题确认

参考库可信度过去主要依赖人工维护 manifest 和文档说明。答辩现场如果追问“样本 ID 是否安全”“获奖样本是否都有官方来源”“本地样本是否已经拉取并记录 HEAD”，需要逐项解释，缺少一条可以直接运行的证据链。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| manifest audit | 新增 `src/os_agent/manifest_audit.py`，检查 `repo_id` 安全、URL、获奖样本奖项和来源、样本目录、HEAD 记录 |
| CLI | 新增 `python scripts\kernelsage.py manifest-audit`，支持 `--json` 和 `--strict` |
| 风险分级 | manifest 字段错误作为 error；本地样本目录或 HEAD 缺失默认作为 warning，`--strict` 可把 warning 作为失败 |
| 回归测试 | 新增 3 条测试覆盖可信样本、不安全 repo_id、缺获奖来源、本地样本缺失和 strict 行为 |
| 运行验证 | 当前内置 manifest 审计结果为 21 个样本、0 error、0 warning |

### 边界说明

- `manifest-audit` 默认不访问网络，只检查本地 manifest 和已采集样本，避免答辩演示受外部网络影响。
- `award_source_url` 如果是远程 URL，只确认已记录；如果是本地相对路径，会在项目目录及外层资料目录中尝试解析。
- `data/samples/<repo_id>/` 是本地生成数据，不提交仓库；因此缺目录或缺 HEAD 默认提示 warning，而不是阻断普通开发流程。
- 新增 3 条回归测试后，完整测试数从 73 个增加到 76 个。

## 阶段 45：新增 HTML 证据报告

- 日期：2026-06-19
- 目标：在 Markdown 报告之外提供更适合答辩展示的 HTML 证据视图，集中呈现结论、证据文件、行号、自检状态和相似度分数。

### 问题确认

Markdown 报告适合版本管理和文本复核，但答辩现场需要更快地展示“结论从哪里来”“证据路径和行号是否有效”“相似度只是线索还是结论”。如果只打开 Markdown，评委需要在章节和代码片段之间来回查找，展示效果和现场可读性都偏弱。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| HTML reporter | 新增 `src/os_agent/html_reporter.py`，生成独立 HTML 文件，无需前端服务即可打开 |
| describe | `describe` 支持 `--html` 和 `--html-out`，默认仍输出 Markdown，HTML 默认写入 `data/reports/html/` |
| compare | `compare` 支持 `--html` 和 `--html-out`，展示相似度分数条、结论分组、证据表和 self-check |
| 批量入口 | `describe-all` 和 `demo` 支持 `--html`，批量生成对应 HTML 报告 |
| 证据自检 | HTML 报告复用 `EvidenceChecker`，把无效证据标记为 `invalid`，避免展示层绕过自检 |
| 回归测试 | 新增 HTML 渲染和 CLI 写出测试，覆盖证据表、自检状态和相似度可视化 |

### 边界说明

- HTML 是 Markdown 之外的展示层，不替代规则版 Markdown 报告，也不改变分析结果。
- 默认不生成 HTML，只有显式传入 `--html` 或 `--html-out` 时写文件，避免普通命令产生额外输出。
- HTML 文件写入 `data/reports/html/`，属于本地生成报告目录，不随仓库提交。
- 新增 HTML 回归测试后，当前完整测试数为 80 个，已通过 `python -m unittest discover -s tests`。

## 阶段 46：新增 query-evidence 证据检索入口

- 日期：2026-06-19
- 目标：把空壳 `indexer.py` 和 `retriever.py` 落地为本地源码证据检索能力，支持输入“调度器/页表/系统调用”等概念后返回相关样本、源码文件、行号和片段。

### 问题确认

描述报告和对比报告适合完整评审，但答辩现场经常会临时追问某个 OS 概念：“你们样本库里哪里有页表实现？”“系统调用证据在哪些仓库？”如果只能重新打开报告或手工搜索源码，演示链路会变慢，也不够体现“可解释检索”的智能体能力。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| indexer | 新增本地样本源码索引构建，读取 Rust/C/C++/Assembly 源码，保留 repo、来源等级、奖项、文件语言等元信息 |
| retriever | 新增中文/英文查询扩展，把“调度器、页表、系统调用”等概念映射到七维 OS 画像关键词 |
| 排序 | 按源码行命中、路径上下文和内核路径优先级打分，返回稳定排序的文件、行号、片段和 matched terms |
| CLI | 新增 `python scripts\kernelsage.py query-evidence "调度器/页表/系统调用" --limit 5`，支持 `--json`、`--repo-id` 和 `--max-files-per-repo` |
| 性能边界 | CLI 使用查询词过滤候选路径，默认每仓库最多索引 300 个源码候选文件；无路径命中时回退通用候选 |
| 文档 | README 和 Demo 文档补充 query-evidence、HTML 报告和 manifest-audit 演示入口 |
| 回归测试 | 新增 3 条测试覆盖中文概念扩展、CLI JSON 输出和 parser 参数 |

### 已验证命令

```powershell
$env:PYTHONPATH='src'; python -m unittest tests.test_retriever tests.test_cli
$env:PYTHONPATH='src'; python scripts\kernelsage.py query-evidence "调度器/页表/系统调用" --limit 5
```

### 边界说明

- `query-evidence` 是本地静态检索，不访问网络、不调用 LLM，也不生成评审裁定。
- 检索结果是“相关源码位置”而不是完整语义证明；最终仍需要结合 describe/compare 报告和人工复核。
- 默认文件上限用于控制大样本库现场耗时；需要更高召回率时可调大 `--max-files-per-repo` 或用 `--repo-id` 限定仓库。
- 当前 21 个本地样本上执行 `query-evidence "调度器/页表/系统调用" --limit 5` 用时约 14.2 秒。

## 阶段 47：新增外层特色与亮点文档

- 日期：2026-06-19
- 目标：在外层答辩材料中新增 `特色与亮点.md`，并在仓库内新增 `docs/HIGHLIGHTS.md`，与 `痛点与解决方案.md` 配套，集中记录本工程的功能亮点、处理方案、演示命令和边界表述。

### 问题确认

已有 `痛点与解决方案.md` 能回答“项目遇到什么问题、如何处理”，但答辩时还需要一份更正向的材料说明“项目做出了哪些特色能力、为什么这些能力有价值、现场如何演示”。如果亮点只散落在 README、Demo 和研发日志中，答辩准备时不便于快速提炼。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| 外层文档 | 新增 `C:\Users\CanhuiBao\Desktop\2026操作系统大赛\特色与亮点.md`，作为答辩材料主文件 |
| 仓库文档 | 新增 `docs/HIGHLIGHTS.md`，作为官方仓库内的亮点说明留档 |
| 内容结构 | 按“特色、处理方案、演示命令、答辩表述”组织亮点 |
| 覆盖范围 | 记录源码证据链、七维 OS 画像、分层样本库、manifest-audit、HTML 报告、query-evidence、LLM 审计 fallback、相似性边界、回归测试、本地优先演示 |
| 演示链路 | 补充 manifest-audit、describe --html、compare --html、query-evidence、llm dry-run 推荐演示顺序 |
| 风险边界 | 明确 query-evidence 是静态检索、LLM 是受控增强、相似线索不是抄袭裁定 |

### 边界说明

- `特色与亮点.md` 是外层答辩材料主文件，`docs/HIGHLIGHTS.md` 是仓库内同步版。
- 文档不引入新的程序逻辑，只沉淀现有能力和答辩口径。
- 后续如果新增功能亮点，应同步更新该文档，避免 README、Demo 和答辩材料表述不一致。

## 阶段 48：优化 README 基本信息状态表展示

- 日期：2026-06-19
- 目标：调整 README “一、基本信息”中第二个状态表的前两列宽度，避免在 GitLab/GitHub 渲染时被过长说明列挤压后产生不必要换行。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| README | 将普通 Markdown 状态表改为 HTML 表格 |
| 列宽 | 使用 `colgroup` 为“维度”和“状态”列设置固定宽度 |
| 换行控制 | 前两列使用 `nowrap`，避免 `MVP 闭环`、`LLM 接入`、`已完成` 等短文本被拆行 |
| 兼容性 | 保留第三列自动伸缩，长说明仍可正常换行 |

### 边界说明

- 本次仅调整 README 展示样式，不改变项目功能、命令或测试结论。
- HTML 表格用于控制列宽；普通 Markdown 表格无法可靠指定列宽。

## 阶段 49：收敛 LLM 审计边界与模型配置

- 日期：2026-06-23
- 目标：修复真实 LLM 试跑后发现的审计边界问题，并将默认模型配置调整为 `deepseek-v4-pro`。

### 已完成任务

| 模块 | 完成内容 |
| --- | --- |
| LLM 配置 | 默认模型、`.env.example` 和 README 示例统一改为 `deepseek-v4-pro`；本地 `.env` 仅替换 `LLM_MODEL`，未输出密钥 |
| LLM 审计 | `audit-llm-report` 支持 `--report-type auto/profile/compare`，描述报告不再误报对比章节 |
| 核验摘要 | profile/compare 缺少“核验摘要”按 error 处理；生成链路会用本地 self-check 兜底补全摘要 |
| 复核边界 | compare 报告缺少“待人工复核项”时自动补充边界说明 |
| 误杀修复 | 审计器允许“不构成抄袭裁定”等否定语境，仍拦截肯定式抄袭裁定 |
| 报告口径 | self-check 统计统一表述为“未确认关键结论数”，避免和“仍需人工确认的信息”混写 |
| HTML 边界 | README、亮点文档和痛点文档明确：`--use-llm` 可增强 Markdown，HTML 仍是结构化证据视图 |
...
$env:PYTHONPATH='src'; python scripts\kernelsage.py describe data\samples\xv6-public --repo-id xv6-public --use-llm --html
$env:PYTHONPATH='src'; python scripts\kernelsage.py compare data\samples\xv6-public --repo-id xv6-public --limit 2 --jobs 4 --use-llm --html
