...
- golden 样例已固定 1 份描述报告和 1 份对比报告，但仍需在后续迭代中随规则变化维护。
...
| P1 | 维护已固定的 golden 样例，确保报告规则变化后同步复核 |
...
- 已建立样本来源分级口径：普通公开比赛样本只标为 `competition_sample`，不硬标为特奖/一等奖案例；当前已根据官方提供的 `os-funtion-winners.md` 补入 3 个 `verified_award` 一等奖样本。

## 对比库代表性扩展记录

- 日期：2026-06-08
- 目标：在不大规模拉取仓库的前提下，将参考库扩展到 21 个代表性样本，提高未知输入仓库的比较全面性。
...
- 21 个样本仍不是“全覆盖”，因此创新性判断仍应保留置信度和人工复核提示。
- 当前 `verified_award` 已补入 3 个 2024 功能赛道一等奖样本；未标注为 `verified_award` 的比赛样本仍不得称为优秀获奖案例。

## 画像缓存复用记录

- 日期：2026-06-08
- 目标：降低 21 个历史样本下 `compare` 的重复分析成本，避免每次比较都重新扫描、解析和画像全部历史仓库。
